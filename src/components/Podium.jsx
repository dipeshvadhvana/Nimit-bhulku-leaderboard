        ...
        return (
          ...
          >
            ...
            >
              ...
              />
              <div
                className={`relative w-11 h-11 rounded-full flex items-center justify-center font-display font-bold text-lg mb-3 shrink-0 ${style.badge}`}
              >
                {rank}
              </div>
              <p className="relative text-[10px] uppercase tracking-[0.15em] text-muted mb-1 shrink-0">{style.label}</p>
              <h3 className="relative font-display text-lg font-semibold text-text leading-snug break-words shrink-0">{item[nameKey]}</h3>
              {subKey && resolvePath(item, subKey) && (
                <p className="relative text-xs text-muted mt-1 truncate shrink-0">{resolvePath(item, subKey)}</p>
              )}
